...
export interface UserProfileState {
  ...
  // ── Raw categorical strings (set by UI dispatchers, consumed by helpers) ──
  /** Categorical height band, e.g. '170 - 175cm'. Set via SET_HEIGHT_RANGE. */
  heightRange?: string;
  /** Categorical weight band, e.g. '70 - 85kg（正常/健壮）'. Set via SET_WEIGHT_RANGE. */
  weightRange?: string;
  /** Non-substance addictions list (QKT12). Set via SET_ADDICTIONS. */
  addictions?: string[];
  /** Persistence / consistency level from QKD7. */
  consistencyLevel?: string;
  /** Emergency-funds tier (QKB7), consumed by calculateResilienceModifier. */
  emergencyFunds?: 'easy_300k' | 'family_decent_amount' | 'small_loan_only' | 'nothing' | 'unspecified';
  /** Family-support tier (QKB2), consumed by calculateResilienceModifier. */
  familySupport?: 'excellent' | 'decent' | 'distant' | 'toxic' | 'deceased_good' | 'deceased_neutral' | 'unspecified';
  /** Partner-support tier, consumed by calculateResilienceModifier. */
  partnerSupport?: 'high' | 'moderate' | 'low' | 'zero' | 'none_no_partner' | 'unspecified';
  /** Entrepreneur inputs (QKAC1–QKAC4), consumed by calculateBusinessScore. */
  cashFlowStage?: string;
  weeklyHours?: string;
  customerBase?: string;
  entrepreneurChannel?: string;

  // ── SSR trait flags — set by bonus rules, consumed by ssr.* rules ──
  hasGenerationalWealth?: boolean;
  businessScale?: 'empire' | 'large' | 'medium' | 'small' | 'unspecified';
  isPublicFigure?: boolean;
  networkTier?: 'exceptional' | 'strong' | 'moderate' | 'limited' | 'unspecified';
  athleticTier?: 'elite' | 'amateur' | 'casual' | 'none' | 'unspecified';
  hasExtremeEndurance?: boolean;
  geneticHealth?: 'exceptional' | 'normal' | 'unspecified';
  expertiseLevel?: 'world_class' | 'national' | 'regional' | 'local' | 'unspecified';
  isPolymath?: boolean;
